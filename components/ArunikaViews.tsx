...
export function KnowledgeView({ data }: { data: Snapshot }) {
  ...
  return <div className="page-stack" data-tour-panel="knowledge"><PageIntro eyebrow="Knowledge vault" title="Simpan hal yang layak diingat" text="Semua review buku, catatan sesi, highlights, dan takeaway learning dikumpulkan menjadi knowledge vault pribadi." />
...
export function InsightsView({ data, insights, year, setYear }: any) {
  ...
  return <div className="page-stack" data-tour-panel="insights"><PageIntro eyebrow="Recap & insight" title="Lihat perjalananmu secara utuh" text="Recap buku dan learning sesuai tahun, monthly reads, monthly watch, genre, penulis, dan channel paling dominan." button={<select className="year-select" value={year} onChange={(e)=>setYear(Number(e.target.value))}>{years.map((y:any)=><option key={y}>{y}</option>)}</select>} />
